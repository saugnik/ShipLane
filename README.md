...
record; it is superseded, never erased.
...
- **Auth** — sessions are stateless JWTs in an httpOnly cookie, signed with
  `AUTH_SECRET`. `middleware.ts` is a fast redirect for humans, not the security
  boundary: every route handler re-checks the session against the database, so a
  deactivated account or a bumped `tokenVersion` loses access immediately rather
